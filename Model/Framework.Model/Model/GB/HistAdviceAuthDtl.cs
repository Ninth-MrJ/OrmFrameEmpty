...
namespace XYHis.Model
{
	...
	{   
		...
		private int _adviceId;  //医嘱ID
		
		private int _doctorId;  //开嘱医生，BsDoctor.Id
		
		private int _locationId;  //开嘱科室BsLocation.Id
		
		private int _hospId;  //住院病人对应InHosinfo.Id
		
		private int _groupNum;  //组号
		
		private int _itemId;  //项目ID对应Bsitem.id
		
		private int _formId;  //剂型对应BsFormDrug.id
		
		private double _priceIn;  //住院项目单价
		
		private int _unitTakeId;  //使用单位
		
		private int _frequencyId;  //频率对应BsFrequency.Id
		
		private int _usageId;  //用法对应BsUsage.Id
		...
		private int _operId;  //操作员ID,BsUser.Id
		...
		private int _execLocId;  //执行科室BsLocation.id
		
		private int _oriAdviceId;  //
		...
		private int _drugReqId1;  //药房申请id
		
		private int _drugReqId2;  //药房申请id用于处理首日
		
		private int _detailId1;  //费用ID对应Ininvoicedtl.id
		
		private int _detailId2;  //费用ID对应Ininvoicedtl.id用于处理首日
		...
		private int _executeId;  //执行ID
		
		private int _executeId2;  //执行ID用于处理首日
		
		private double _dosage;  //每次量
		
		private int _execDoctorId;  //执行医生
		...
		private int _usageItemId;  //对应bsUsageItem.id
