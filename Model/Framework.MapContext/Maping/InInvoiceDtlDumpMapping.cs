...
namespace Orm.MapContext
{
	...
	/// 住院欠费费用库 - 实体类
	/// </summary>
	public partial class InInvoiceDtlDumpMapping: ClassMap<InInvoiceDtlDump>
	{   
		/// <summary>
		/// 住院欠费费用库 - 实体类
		/// </summary>
		public InInvoiceDtlDumpMapping()
		{
			this.TableName("ININVOICEDTLDUMP").Schema("orm");
			...
			this.Column(p =>p.AuthDtlId).ColumnName("AUTHDTLID");
			...
			this.Column(p =>p.CheckOperId1).ColumnName("CHECKOPERID1");
			  
			this.Column(p =>p.DeleteTime).ColumnName("DELETETIME");
			  
			this.Column(p =>p.DeleteOperId).ColumnName("DELETEOPERID");
			  
			this.Column(p =>p.AuthDtlId1).ColumnName("AUTHDTLID1");
