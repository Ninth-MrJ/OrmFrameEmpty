...
namespace OracleDataToRedis.Coders.Persistence
{
    public class ProjectCoder
    {
        public static void Write(IList<DbTable> dts)
        {
            ...
            EntityDaoCoder.Write();
            EntityDaoTransationCoder.Write();
            SessionFactoryCoder.Write();
        }
        ...
        private static void CreateProject(IList<DbTable> dts)
        {
            ...
            string filepath = Path.Combine(path, BaseParams.PersistenceNameSpace + ".csproj");
            ...
            sw.WriteLine("    <ProjectGuid>{" + BaseParams.PersistenceNo + "}</ProjectGuid>");
            sw.WriteLine("    <OutputType>Library</OutputType>");
            sw.WriteLine("    <AppDesignerFolder>Properties</AppDesignerFolder>");
            sw.WriteLine("    <RootNamespace>" + BaseParams.PersistenceNameSpace + "</RootNamespace>");
            sw.WriteLine("    <AssemblyName>" + BaseParams.PersistenceNameSpace + "</AssemblyName>");
            ...
            sw.WriteLine("  </ItemGroup>");
            sw.WriteLine("  <ItemGroup>");
            sw.WriteLine("    <Compile Include=\"EntityDao.cs\" />");
            sw.WriteLine("    <Compile Include=\"EntityDao.Transation.cs\" />");
            sw.WriteLine("    <Compile Include=\"Properties\\AssemblyInfo.cs\" />");
            sw.WriteLine("    <Compile Include=\"SessionFactory.cs\" />");
            sw.WriteLine("  </ItemGroup>");
            sw.WriteLine("  <ItemGroup>");
            ...
            AssemblyInfoCoder.Write(path, BaseParams.PersistenceNameSpace, BaseParams.PersistenceNo);
        }
        #endregion
