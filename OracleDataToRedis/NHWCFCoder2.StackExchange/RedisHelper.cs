...
using XYHis.Model;
using XYHis.ServiceStackRedisHelper;

namespace OracleDataToRedis.StackExchange
{
    ...
    public class RedisHelper
    {
        ...
        static RedisHelper()
        {
            ...
            ConnectionString = section.WriteServerList;
        ...
        public RedisHelper()
        {
            ...
            try
            {
                /*以下这段代码的作用是通过反射，每五个项目为一组确定其基础数据导入redis时数据库序号，redis总共有16个数据库,其序号为0至15*/
                Type enumType = typeof(EnumClass.GPProjectsEnum);
                FieldInfo[] fields = enumType.GetFields();
                foreach (FieldInfo fieldInfo in fields)
                {
                    if (fieldInfo.Name.ToLower() == DefaultKey.ToLower())
                    {
                        fieldValue = (int)fieldInfo.GetRawConstantValue();
                        break;
                    }
                }
                if (fieldValue == -1)
                ...
                {
                    int remainder = fieldValue / 5;//余数
                    int modulu = fieldValue % 5; //模数
                    if (modulu != 0)
                    {
                        dbIndex = remainder;
                    }
            ...
            _db = ConnMultiplexer.GetDatabase(dbIndex);
            //_db = ConnMultiplexer.GetDatabase(int.Parse(ConfigurationManager.AppSettings["dbIndex"].ToString()));
        }
        ...
        {
            listKey = listKey.Select(x => AddKeyPrefix(x)) as List<RedisKey>;
            return _db.StringGet(listKey.ToArray());
        }
