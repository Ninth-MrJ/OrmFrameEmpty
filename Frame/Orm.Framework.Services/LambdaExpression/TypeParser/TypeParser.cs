...
namespace Orm.Framework.Services.LambdaExpressionHelper.Core
{
    ...
    public class TypeParser
    {
        ...
        {
            Type type = null;
            StringBuilder sbValue = new StringBuilder(string.IsNullOrEmpty(typeName) ? spResult.Next() : typeName);
            do
            {
                ...
                if (spResult.PeekNext() == "<")
                {
                    ...
                    sbValue.AppendFormat("`{0}[{1}]", listGenericType.Count, string.Join(",", listGenericType.Select(p => "[" + p.AssemblyQualifiedName + "]").ToArray()));
                }
