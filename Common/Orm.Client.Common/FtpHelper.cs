...
namespace Orm.Client.Common
{
    ...
    public class FTPHelper
    {
        ...
        /// <param name="RemoteFileName">远程文件名</param>
        /// <param name="LocalFullPath">本地完整路径文件名</param>
        public void DownloadFileAsync(string RemotePath, string RemoteFileName, string LocalFullPath, bool OverWriteLocalFile = false)
        {
            try
            {
                if (!IsValidFileChars(RemoteFileName))
                ...
                if (File.Exists(LocalFullPath) && !OverWriteLocalFile)
                {
                    throw new Exception("当前路径下已经存在同名文件！");
                }
                MyWebClient client = new MyWebClient();
                ...
                client.DownloadFileAsync(new Uri(this.Uri.ToString() + RemotePath + RemoteFileName), LocalFullPath);
            }
            catch (Exception ep)
            {
                ErrorMsg = ep.ToString();
        ...
        /// </summary>
        /// <param name="LocalFullPath">本地带有完整路径的文件名</param>
        /// <param name="RemotePath">远程目录，如:SubDirectory1/SubDirectory2</param> 
        /// <param name="RemoteFileName">要在FTP服务器上面保存文件名</param> 
        /// <param name="OverWriteRemoteFile">是否覆盖远程服务器上面同名的文件</param>
        public void UploadFileAsync(string LocalFullPath, string RemotePath, string RemoteFileName, bool OverWriteRemoteFile)
        {
            try
            {
                if (!IsValidFileChars(RemoteFileName) || !IsValidFileChars(Path.GetFileName(LocalFullPath)) || !IsValidPathChars(Path.GetDirectoryName(LocalFullPath)))
                ...
                { 
                    ...
                    client.UploadFileAsync(new Uri(this.Uri.ToString() + RemotePath + RemoteFileName), LocalFullPath);
        ...
        public void DeleteFile(string RemotePath, string RemoteFileName)
        {
            try
            {
                ...
                Response = Open(new Uri(this.Uri.ToString() + RemotePath + RemoteFileName), WebRequestMethods.Ftp.DeleteFile);
            }
            catch (Exception ep)
            {
                ErrorMsg = ep.ToString();
        ...
        /// <returns></returns>
        public bool FtpFileExist(string RemotePath, string FileName)
        {
            var fileList = GetFileList(RemotePath);
